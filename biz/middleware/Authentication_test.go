...
func TestAuthMiddlewareRight(t *testing.T) {
	// 初始化测试数据库
...
func TestAuthMiddlewareRedis(t *testing.T) {
	// 初始化测试数据库
	gin.SetMode(gin.TestMode)
	err := model.TestDBInit()
	if err != nil {
...
func TestAuthMiddlewareLogInfo(t *testing.T) {
	// 初始化测试数据库
	gin.SetMode(gin.TestMode)
	err := model.TestDBInit()
	if err != nil {
...
func TestAuthMiddlewareStaff(t *testing.T) {
	// 初始化测试数据库
	gin.SetMode(gin.TestMode)
	err := model.TestDBInit()
	if err != nil {
	...
	s := miniredis.RunT(t)
	// set some keys like your codes expected
	loginInfo := `{"id": 1, "username": "fuc_sun", "is_staff": 0}`
	err = s.Set("woshisun", loginInfo)
	if err != nil {
		t.Errorf("set redis error: %s", err.Error())
	}
